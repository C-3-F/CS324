...
int main(int argc, char *argv[]) {
  ...
  /* Check usage */
  if (!(argc == 2 || (argc == 3 && (strcmp(argv[1], "-4") == 0 ||
                                    strcmp(argv[1], "-6") == 0)))) {
  ...
  while (1) {
    ...
    while (1) {
      ...
      ssize_t nread = recv(conn, buf, BUF_SIZE, 0);
      if (nread == 0) {
        close(conn);
        break;
      }
      ...
      // Extract the IP address and port from remote_addr using parse_sockaddr().  parse_sockaddr() is defined in
      // ../code/sockhelper.c.
      parse_sockaddr(remote_addr, remote_ip, &remote_port);
      printf("Received %zd bytes from %s:%d\n", nread, remote_ip, remote_port);

      if (send(conn, buf, nread, 0) < 0) {
